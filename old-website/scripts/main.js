...
jQuery(document).ready(function($) {
	...
	$(".slider").carouFredSel({
		...
		scroll: {
			...
			onAfter: function(data) {
				jQuery("#" + jQuery(data.items).attr("id") + "_content").fadeIn();
				jQuery(".slider_navigation .more").css("display", "none");
				jQuery("#" + jQuery(data.items.visible[0]).attr("id") + "_content").css("display", "block");
			}
		},
		pagination: {
		...
		auto: {
			...
			timeoutDuration: parseInt(config.slide_interval),
			fx: config.slider_effect,
			easing: config.slider_transition,
			duration: parseInt(config.slider_transition_speed)
		}
	...
	$(".upcoming_classes").carouFredSel({
		...
		prev: {
			button: '#upcoming_class_prev',
			timeoutDuration: 7000
		},
		next: {
			button: '#upcoming_class_next',
			timeoutDuration: 7000
		},
		auto: {
			play: true,
	...
	$(".tabs .ui-tabs-nav a").click(function() {
		if ($(this).attr("href").substr(0, 4) != "http") $.bbq.pushState($(this).attr("href"));
	...
	$(window).bind("hashchange", function(event) {
		...
		if (hashSplit[0].substr(0, 7) != "filter=" && hashSplit[0].substr(0, 4) != "page") {
			...
			var hashString = "";
			for (var i = 0; i < hashSplit.length - 1; i++) hashString = hashString + hashSplit[i] + (i + 1 < hashSplit.length - 1 ? "-" : "");
		...
		if (typeof(hashOptions.filter) != "undefined") {
			...
			if ($('.isotope_filters a[href="#filter=' + hashOptions.filter + '"]').length) $('.isotope_filters a[href="#filter=' + hashOptions.filter + '"]').addClass("selected");
			...
			if (parseInt($("#comment_form [name='prevent_scroll']").val()) == 1) {
				...
				$.ajax({
					...
					success: function(json) {
						if (typeof(json.html) != "undefined") $("#comments").html(json.html);
						var hashSplit = location.hash.split("/");
						var offset = null;
						if (hashSplit.length == 2 && hashSplit[1] != "") offset = $("#" + hashSplit[1]).offset();
						else offset = $("#comments").offset();
						if (offset != null) $("html, body").animate({
							scrollTop: offset.top - 10
						}, 400);
						$("#comment_form [name='paged']").val(parseInt(location.hash.substr(6)));
					}
		...
		if (location.hash.substr(1, 21) == "gallery-details-close" || typeof(hashOptions.filter) != "undefined") {
			...
			if (!visible) {
				$(".gallery_item_details_list").css("display", "block").animate({
					...
					if (typeof(json.submit_message) != "undefined" && json.submit_message != "") {
						...
						if (id == "comment_form" && typeof(json.html) != "undefined") {
							...
							if (typeof(json.comment_id) != "undefined" && $("#comments").children().length) {
								...
								if (typeof(json.change_url) != "undefined" && $.param.fragment() != json.change_url.replace("#", "")) $("#comment_form [name='prevent_scroll']").val(1);
							}
							if (typeof(json.change_url) != "undefined") window.location.href = json.change_url;
						}
						$("#" + id)[0].reset();
